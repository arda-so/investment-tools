...
# Resolve mutable tag references to immutable digests for reproducible job executions.
if [[ "$IMAGE" != *@sha256:* ]] && [[ "$IMAGE" == *:* ]]; then
  IMAGE_REPO="${IMAGE%:*}"
  IMAGE_TAG="${IMAGE##*:}"
  IMAGE_DIGEST="$(gcloud artifacts docker tags list "$IMAGE_REPO" \
    --filter="tag:${IMAGE_TAG}" \
    --format='value(version)' | head -n1 || true)"
...
# Unified ontology maintenance: decay refresh + canonicalization + anti-reverify.
ONTOLOGY_MAINTENANCE_SCHEDULE="${ONTOLOGY_MAINTENANCE_SCHEDULE:-15 2 * * *}"
deploy_job "investor-ontology-maintenance-job" \
  "/app/bin/run_ontology_maintenance_job" "" \
  "${ONTOLOGY_MAINTENANCE_SCHEDULE}" "2400s"
